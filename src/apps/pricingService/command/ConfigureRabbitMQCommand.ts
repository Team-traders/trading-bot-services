...
export class ConfigureRabbitMQCommand {
  static async run() {
    const connection = container.get<RabbitMqConnection>('PricingService.Shared.RabbitMQConnection');
    const { name: exchange } = container.get<RabbitMQConfig>('PricingService.Shared.RabbitMQConfig').exchangeSettings;
    await connection.connect();

    const configurer = container.get<RabbitMQConfigurer>('PricingService.Shared.RabbitMQConfigurer');
